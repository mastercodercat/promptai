    ...
    <div class="flex flex-rows p-16px gap-sm justify-between">
        <div class="flex flex-rows">
            <div class="flex flex-rows gap-sm items-center">
                ...
                <CommonSelect title="OpenAI" :value="selectedModel" :items="group_items" :select="onGroupSelect" />
            </div>
        ...
        <div class="flex flex-rows gap-sm items-center">
            <div>
                <span class="border border-grey-border rounded-lg p-1 text-xs text-grey-text">Ctrl</span>
                <span class="text-xs text-grey-text px-1">+</span>
                <span class="border border-grey-border rounded-lg p-1 text-xs text-grey-text">Enter</span>
            </div>
...
export default {
    ...
    computed: {
        group_items() {
            return this.model_groups.map(item => ({key: item.id, value: item.name}));
        },
        model_items() {
            return this.models.filter(model => model.groupId === this.selectedModel).map(item => ({key: item.id, value: item.name}));
        }
    },
    methods: {
        onGroupSelect(ev) {
