<template>
    <div class="flex flex-col bg-dark-bg p-24px gap-sm">
        ...
        <div class="rounded-[8px] bg-editor text-white p-24px leading-editor whitespace-pre-wrap">
            <div :contentEditable="isEditing" aria-multiline="true" @keyup="onEditorChanged" id="editor" @focusout="onEditEnd" role="textbox" spellcheck="false" class="bg-transparent text-white w-full" :on-change="onEditorChanged" v-if="isEditing">
                {{ editorContent }}
            </div>
            <div class="whitespace-pre-wrap" @click="onEdit" v-else>
                ...
                <span class="float-left rounded-prompt px-3 border border-white">Completion</span>
            </div>
        </div>
        <div class="text-white">
            Tokens: {{promptContent.split(" ").length}}
        </div>
    </div>
</template>
